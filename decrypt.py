...
# TO DO:
# 1.) Fix file location input - DONE
# 2.) Upload to github <- done-ish (not updated version)
# 3.) Implement prime number thing
# 4.) Test ALL inputs (validation)
...
def findPrime(n):
    # n is the number of digits our prime # is
    # this will get us a (semi) random number
    next_prime = random.randint(0, 10) * pow(10, (n - 1))
    return find_next_prime(next_prime)


def findPandQ(n):
    # NOTE: this is going to temporarily replace the findPrime function
    #prime1 = Crypto.Util.number.getPrime(n)
    #prime2 = Crypto.Util.number.getPrime(n)
    prime1 = sympy.nextprime(random.getrandbits(n))
    prime2 = sympy.nextprime(random.getrandbits(n))
    if prime1 != prime2:
...
def find_next_prime(n):
    return find_prime_in_range(n + 1, 2 * n)
...
def newFile():
    ...
    while True:
        ...
        while not os.path.isdir(file_location):
    ...
    bit_length = input()
    while not bit_length.isnumeric():
        print("not a valid input")
        print("how many bits would you like the p&q numbers to be?")
        bit_length = input("(recommended size is 1024)")
    """
    # n = int(input())
...
def processFile(file_location):
    ...
    if (len(public_key) == 2):
        ...
        # returning false so editFile() function can stop and return when file
    ...
    while True:
        ...
        if choice == "e":
            ...
            while d.isnumeric() != True:
                ...
                d = input()
            print_file = file_location.split("/")
            print("\033[1;32;10mprinting file '\033[1;33;10m%s\033[1;32;10m' (decrypted)" % print_file[
                len(print_file) - 1])
...
def addToFile(file_location, public_key):
    ...
    while (confirmation == "y") != True:
        ...
        confirmation = input()
    print("\033[1;32;10madded '\033[1;33;10m%s\033[1;32;10m' to '\033[1;33;10m%s\033[1;32;10m'" % (
        text_to_add, file_location))
...
def editFile():
    ...
    # WE MAY JSUT HAVE TO SCRAP AND DO FULL FILE LOCATION
    #  SHORTCUT FOR TESTING IS NOT WORKING WELL
    while (public_key == False):
        validFile = False
        ...
        while (validFile == False):
            ...
            if (os.path.isfile(file_location) == True):
                ...
                print("\033[1;31;10mfile not found, please try again\033[1;32;10m")
        print("FILE PATH: \033[1;31;10m%s\033[1;32;10m" % file_location)
        public_key = processFile(file_location)

        if (public_key == False):
            print("\033[1;31;10mfile was not in the correct format\033[1;32;10m")
...
def createLocationFile():
    # this function will first ask if user has moved location file
    #  (if it previously existed) then check location file location based on
    #  response. If no location file exists, create one and write file locations
    #  to newfound location file. if the file does exist
